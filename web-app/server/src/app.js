...
app.post("/registerVoter", async (req, res) => {
  ...
  var str = [];
  str.push(voterId.substring(0,2))
  str.push(req.body.registrarId.substring(0,4))
  str.push(voterId.substring(2,4))
  str.push(req.body.registrarId.substring(4,8))
  str.push(voterId.substring(4,8))
  str.push(req.body.registrarId.substring(8,12))
  str.push(voterId.substring(8,15))
  ...
  if (response.error) {
    ...
    if (invokeResponse.error) {
      ...
      let parsedResponse = "Your identity has been created successfully. please use "+ str.join('') + " to login on portal";
      

      var transporter = nodemailer.createTransport({
        service: "gmail",
        auth: {
          user: "jp30101995.aws2@gmail.com",
          pass: "Jimmy@3010"
        }
      });
